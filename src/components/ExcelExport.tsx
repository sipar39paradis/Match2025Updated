...
const exportToExcel = async (excelData: any[], fileName: string) => {
  const fileType =
    'application/vnd.openxmlformats-officialdocument.spreadsheetml.sheet;charset=UTF-8';
...
export const personalInformationAsExcel = (
  ...
  const asJson = {
    ...
    PostalProvince: questionnaire?.contactDetails?.isDifferentProvince
      ? questionnaire?.contactDetails?.differentProvince
      : ProvinceEnum.QUEBEC,
  ...
  return exportToExcel(
    [asJson],
    questionnaire?.personalInformations?.firstName +
      '_' +
      questionnaire?.personalInformations?.lastName
  );
};
