...
export interface AppContextType {
  ...
  signInWithGoogle: () => Promise<[Promise<string>, MultiFactorResolver, string]>;
  signInWithFacebook: () => Promise<string>;
  enrollTwoFactor: (phoneNumber: string) => Promise<string>;
  verifyEnrollingTwoFactor(promise: Promise<string>, verificationCode: string): Promise<void>;
  verifyTwoFactor(promise: Promise<string>, verificationCode: string, resolver: MultiFactorResolver): Promise<void>;
  signOut: () => void;
...
export function AppContextProvider({ children }: AppContextProviderProps) {
  ...
  async function enrollTwoFactor(phoneNumber: string): Promise<string> {
    ...
    const recaptchaVerifier = new RecaptchaVerifier('two-factor-button', {
      'size': 'invisible',
  ...
  const verifyEnrollingTwoFactor = async (promise: Promise<string>, verificationCode: string) => {
  ...
  const verifyTwoFactor = async (promise: Promise<string>, verificationCode: string, resolver: MultiFactorResolver) => {
    promise.then(function (verificationId) {
  ...
  async function signInWithGoogle(): Promise<[Promise<string>, MultiFactorResolver, string]> {
    ...
    await signInWithPopup(auth, provider)
      ...
      .catch((error) => {
        ...
        const recaptchaVerifier = new RecaptchaVerifier('google-login', {
          'size': 'invisible',
          'callback': function(response) {
          ...
          if (resolver.hints[selectedIndex].factorId ===
              ...
              const phoneAuthProvider = new PhoneAuthProvider(auth);
              // Send SMS verification code
              promise = phoneAuthProvider.verifyPhoneNumber(phoneInfoOptions, recaptchaVerifier)
          } else {
