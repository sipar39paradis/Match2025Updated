...
export const formSchema = Yup.object().shape({
  personalInformations: Yup.object({
    firstName: Yup.string().required('Le prénom est requis'),
    lastName: Yup.string().required('Le nom de famille est requis'),
    email: Yup.string()
      .email()
      .min(6, "L'adresse courriel doit être au moins 6 caractères"),
    socialInsuranceNumber: Yup.string().required('Le numéro d\'assurance social est requis').matches(sinRegExp, 'Le format doit être XXX-XXX-XXX'),
